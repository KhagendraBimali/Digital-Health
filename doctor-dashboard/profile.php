...
if($_SERVER["REQUEST_METHOD"]=="POST"){
    ...
    $query = 'UPDATE doctor_signup SET full_name=?, address=?, speciality=?, phone=?, profile=? WHERE doctor_id=?';
    $stmt = $conn->prepare($query);
    $stmt->bind_param('ssssss', $full_name, $address, $speciality, $phone, $targetProfile, $doctorId) ;
    $stmt->execute() ;
    $stmt->close();
...
$query = "SELECT * FROM doctor_signup WHERE doctor_id =?";
$stmt = $conn->prepare($query);
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        echo '<label><strong>Name: </strong> <span id="nameDisplay">Dr. ' . $row['full_name'] . '</span><input type="text" name="full_name" value="' . $row['full_name'] . '" style="display:none;"></label><br>';
        echo '<label><strong>Qualification: </strong><h4>' . $row['qualification'] . '</h4></label><br>';
        echo '<label><strong>Speciality: </strong> <span id="specialityDisplay">' . $row['speciality'] . '</span><input type="text" name="speciality" value="' . $row['speciality'] . '" style="display:none;"></label><br>';
        ...
        echo '<label><strong>Phone: </strong> <span id="phoneDisplay">' . $row['phone'] . '</span><input type="text" name="phone" value="' . $row['phone'] . '" style="display:none;"></label><br>';
        echo '</div>';
        echo '<div id="profile">';
